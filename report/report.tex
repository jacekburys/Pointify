...
Over the course of 3 months we have endured and have managed to produce a stable application, that combines various technologies together such as angularJS and nodeJS, to successfully 3D reconstruct a scene. Through the many problems and setbacks we encountered such as deciding to do a fresh project on Linux which took some time setting up as well as figuring out the calibration of the Kinect cameras, we persevered by using our knowledge and past experience to (explain a little about the problem with the calibration)/
\\\\
For some members, they were already familiar with the technology we were using (angularJS and nodeJS) but for other members they had a change to accquire 
