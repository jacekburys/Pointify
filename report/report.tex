...
Pointify (coming from the idea that an object or a scene can be reconstructed by rendering many points together from captured depth information) was built over the course of 3 months. During this short time we have endured and have managed to produce a stable application, that combines various technologies together such as angularJS and nodeJS, to successfully capture depth information through multiple Kinect cameras, perform calibration and then rendering the many points together to 3D reconstruct a scene. Through the many problems and setbacks we encountered such as deciding to do a fresh project on Linux which took some time setting up as well as figuring out the calibration of the Kinect cameras, we persevered by using the knowledge we attained through our term courses, Computer Vision ... and past experience to (explain a little about the problem with the calibration)/
\\\\
For some members, they were already familiar with the technology we were using (angularJS and nodeJS) so they were able to refine their own skills but for other members they had a chance to learn how to use angularJS and nodeJS (could be phrased better).   
...
%The very next step we have planned is to release the project on GitHub once it is officially completed.
%We are planning to open source all of our code including instructions regarding how to build and use the
%system as well as description of the overall structure of the code.
%We hope that not only will we receive valuable feedback and (hopefully as few as possible) bug reports,
%but we also will be able to collaborate with developers on system enhancements and system applications
%they might want to create. We also hope that projects such as ours might encourage the libfreenect2
%developers to improve their product in terms of stability and available API.
