...
There are features that we believe can improve the usability of this product, but didn't implement due to the harsh time limit on the project. We managed to identify some top ranking future extensions and listed them below. Hopefully we can have a slim chance to implement them in the future.
\\\\
1. We can try to build up GUI implementation on client side. This will allow people who don't have technical background to use the product with ease, instead of looking up "What is command line?". This will allow us to integrate more things with ease, for example, we can simply display the current frame rate on the screen. We can try to use some platform-independent UI library to achieve this, possibly Qt.
\\\\
2. We can try to make the product work with more types of cameras. There might be other types of cameras that can capture depth information. What we can do is to build up a layer of indirection on the hardware driver, so that the current existing calculation code will work with different camera implementations. We also need to unify the format data that is passed so that we can maximise the amount of code that can be reused.
