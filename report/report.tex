...
The ability to take an image and generate a 3D point cloud in real time, with multiple calibrated cameras, giving both depth and colour information has multiple use cases today. Through Pointify we have developed a system that utilises Microsoft's proprietary Xbox Kinect hardware to stream these 3D point clouds based on the XYZ and RGB data from the sensors on the devices in real time, allowing us to reconstruct a live 3D model of a scene, that updates live. It is a truly cross-platform service which allows the camera clients and the server to run on Windows, OS X or Linux operating systems. We achieve this by using services and programming languages that run on all operating systems. Having multiple calibrated Kinect cameras allows for a full 360 degree view of the object or scene that is being photographed. 
\\\\
The system works by:
\\\\
Client
...
\item Uses a ThreeJS renderer to display the received point clouds in real-time.
...
for example to create 3D models for VR games, manufacturing, 3D printing and medical applications. Gathering the points is done using one or more 3D scanners, calibrated with each other. Microsoft Kinect is one such device which is widely available and affordable, and includes both a depth and colour sensor as required. Pointify is an application that makes use of multiple Kinects to easily create aligned point clouds, as well as providing a web interface to view and export them for use in other applications.
...
\begin{itemize}
  ...
  \item Allow that the set up and calibration of the cameras is simple.
  % TODO : anything else?
\end{itemize}
